...
internal object ArchitectureRendering {
    fun violations(model: ArchitectureModel, edges: List<DependencyEdge>): List<String> =
        edges.mapNotNull { edge ->
            ...
            if (ArchitectureModelBuilder.isIgnored(edge.sourcePath, model.ignoredProjectPaths)) return@mapNotNull null
            if (ArchitectureModelBuilder.isIgnored(edge.targetPath, model.ignoredProjectPaths)) return@mapNotNull null
            if (AllowedEdge(edge.sourcePath, edge.targetPath) in model.allowances) return@mapNotNull null
    ...
    fun report(model: ArchitectureModel): String = buildString {
    ...
    private fun violation(
        ...
        return buildString {
            ...
            appendLine("- Reverse or invert the dependency so the '${target.layer.name}' layer does not own a dependency required by '${source.layer.name}'.")
            appendLine("- Reconsider whether ':${target.projectRoot}' belongs in the '${target.layer.name}' layer.")
            append("- Add a narrow documented exception only when the violation is intentional and temporary.")
        }
    }
}
