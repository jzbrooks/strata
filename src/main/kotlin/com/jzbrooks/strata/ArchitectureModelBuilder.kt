...
internal object ArchitectureModelBuilder {
    ...
    fun build(
        ...
        val layers = extension.layers().mapIndexed { index, spec ->
            ...
            if (rawRoots.isEmpty()) {
                errors += "Architectural layer '${spec.name}' must declare at least one top-level project root."
            }
            ...
            rawRoots.forEach { raw ->
                ...
                if (!normalizedRoots.add(normalized)) {
                    errors += "Top-level project root ':$normalized' is declared more than once in architectural layer '${spec.name}'."
                }
                rootAssignments.getOrPut(normalized) { mutableListOf() } += spec.name
                if (":$normalized" !in projectPaths) {
                    errors += "Top-level project root ':$normalized' configured for architectural layer '${spec.name}' does not exist in this build."
                }
            }
            LayerDefinition(spec.name, index, normalizedRoots)
        }

        rootAssignments.filterValues { it.size > 1 }.forEach { (root, assignedLayers) ->
            errors += duplicateRootMessage(root, assignedLayers)
        }

        val ignoredPaths = extension.ignoredProjectPaths.orNull.orEmpty().toList().mapNotNullTo(linkedSetOf()) { path ->
            validateExistingProjectPath(path, "Ignored project path", projectPaths, errors)
        }
        val ignoredConfigurations = extension.ignoredConfigurationNames.orNull.orEmpty().toList().toSet()
        ignoredConfigurations.filter(String::isBlank).forEach {
            errors += "Ignored configuration names must not be blank."
        }

        val allowances = extension.allowances().mapNotNullTo(linkedSetOf()) { allowance ->
            if (allowance.because.isBlank()) {
                errors += "Architecture exception from '${allowance.from}' to '${allowance.to}' must include a non-blank justification."
            }
            val from = validateExistingProjectPath(allowance.from, "Architecture exception source", projectPaths, errors)
            val to = validateExistingProjectPath(allowance.to, "Architecture exception target", projectPaths, errors)
        ...
        if (extension.unclassifiedProjects.get() == UnclassifiedProjectPolicy.FAIL && unclassified.isNotEmpty()) {
            errors += buildString {
                appendLine("The following projects are not assigned to an architectural layer:")
                unclassified.sorted().forEach { appendLine("  $it") }
                append("Assign their top-level roots to a layer, ignore their project subtrees, or set unclassifiedProjects to IGNORE.")
            }
        }
    ...
    fun isIgnored(path: String, ignoredPaths: Set<String>): Boolean =
        ignoredPaths.any { path == it || path.startsWith("$it:") }

    private fun normalizeProjectRoot(raw: String, layerName: String, errors: MutableList<String>): String? {
        if (raw.isBlank()) {
        ...
        if (normalized.isBlank() || ':' in normalized || normalized.any(Char::isWhitespace)) {
            errors += "Project root '$raw' in architectural layer '$layerName' is not a top-level project root; use a value such as 'app' or ':app'."
            return null
        }
        return normalized
    }
    ...
    private fun validateExistingProjectPath(
        ...
        if (!absoluteProjectPath.matches(path)) {
            errors += "$label '$path' is malformed; use an absolute non-root Gradle project path such as ':app' or ':app:feature'."
            return null
        }
