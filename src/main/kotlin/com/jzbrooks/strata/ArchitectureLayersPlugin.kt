...
class ArchitectureLayersPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        if (project != project.rootProject) {
            throw GradleException("The com.jzbrooks.strata plugin must be applied to the root project only.")
        }
        ...
        project.gradle.projectsEvaluated {
            ...
            val includedProjects = project.allprojects
                .filter { it != project }
                .map { candidate ->
                    val relativeBuildFile = runCatching {
                        rootDirectory.relativize(candidate.buildFile.toPath()).toString().replace('\\', '/')
    ...
    private fun collectEdges(rootProject: Project): List<DependencyEdge> = buildList {
        rootProject.allprojects.filter { it != rootProject }.forEach { source ->
            val relativeBuildFile = runCatching {
                rootProject.rootDir.toPath().relativize(source.buildFile.toPath()).toString().replace('\\', '/')
