...
class ArchitectureRenderingTest {
    ...
    private val platform = LayerDefinition("platform", 3, linkedSetOf("infrastructure", "networking", "database"))
    ...
    fun `forbidden diagnostic identifies logical layers and matched roots`() {
        ...
        val diagnostic = ArchitectureRendering.violations(
            model,
    ...
    fun `report groups sibling roots and nested projects by logical layer`() {
        ...
        assertContains(report, "   May depend on:\n     application\n     domain\n     data\n     platform")
    }

    private fun model(vararg classifications: ProjectClassification): ArchitectureModel = ArchitectureModel(
        layers = layers,
        layersByProjectRoot = layers.flatMap { layer -> layer.projectRoots.map { it to layer } }.toMap(),
