...
class ArchitectureModelBuilderTest {
    ...
    fun `normalizes roots and classifies nested sibling projects`() {
        ...
        val model = ArchitectureModelBuilder.build(
            extension,
            identities(":app", ":app:main", ":features", ":features:checkout", ":domain"),
        )

        assertEquals("application", model.classificationsByPath.getValue(":features:checkout").layer.name)
    ...
    fun `rejects normalized duplicates in one layer`() {
        ...
        assertContains(failure.message.orEmpty(), "declared more than once in architectural layer 'application'")
    }
    ...
    fun `rejects roots assigned to multiple logical layers`() {
        ...
        assertContains(failure.message.orEmpty(), "Top-level project root ':features' is assigned to multiple architectural layers")
    ...
    private fun identities(vararg paths: String) = paths.map { ProjectIdentity(it, "${it.removePrefix(":")}/build.gradle.kts") }
}
